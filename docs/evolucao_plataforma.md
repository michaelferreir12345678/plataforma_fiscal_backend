...
| Data | Alteração |
|---|---|
| 2026-08-13 | **Bug real de produção corrigido: o Assistente falhava desde que ganhou ferramentas.** Reportado pelo usuário: `400 INVALID_ARGUMENT — Function call is missing a thought_signature in functionCall parts`. **Causa medida, não suposta:** o SDK fixado (`google-genai==1.16.1`) não conhece o campo `Part.thought_signature` — provado com `'thought_signature' in types.Part.model_fields`, que devolve `False` no SDK antigo e `True` na 1.75.0. O modelo `gemini-3.5-flash` tem raciocínio e **exige** que a assinatura volte junto do histórico na segunda volta do laço de ferramentas; o SDK a descartava ao desserializar, por ser campo desconhecido. Como o *function calling* só entrou na Sprint IA-1a, o campo nunca fora exigido antes — o defeito nasceu com a funcionalidade, não com o SDK. **A correção é só o pin** (`1.75.0`, a última 1.x, escolhida em vez da 2.18 para resolver com o menor raio de impacto): o laço já devolvia o `content` do modelo intacto, faltava um SDK que preservasse o campo. O upgrade atravessa 59 versões menores e serve chat **e** *embeddings*, então o raio foi medido com a suíte inteira — verde. **O guardrail §9 funcionou no primeiro erro real da camada nova:** a falha do provedor virou `LLMProviderError` (RFC 7807) e o assistente **recusou em vez de inventar**, que é exatamente o comportamento que a Sprint 17 desenhou. |
| 2026-08-13 | **Correção da E1 em produção — a eleição do veredito vigente deixou de ser sorteio (migration `0044`).** Encontrado durante a verificação da Sprint IA-1a, não por relato: a suíte completa reprovava `test_retificacao_cria_linha_nova_e_nao_apaga_o_veredito_anterior` de forma intermitente. **O defeito era meu, da E1.** A eleição do veredito vigente por chave usava `executado_em` mais recente com desempate por `id` — um `uuid4()` —, e o comentário do próprio `upsert_check` descartava o risco com a premissa de que o relógio da aplicação sempre distinguiria duas execuções. **A premissa é falsa onde o relógio é grosso:** medido nesta base, no Windows, **200 chamadas consecutivas de `datetime.now(UTC)` devolveram o mesmo valor**. Empatado o carimbo, quem vencia era o UUID. **Alcance medido antes de corrigir** (a régua do §12: medir, não estimar): dev, 31 de 193 linhas empatadas por chave; **produção, 230 grupos empatados em 1.283 linhas** — proporcionalmente mais que o dev, contrariando a hipótese inicial de que o relógio de microssegundo do Linux tornaria a colisão rara (a causa real é outra: os `freshness_*` são gravados em lote, e `now()` do PostgreSQL é o instante da **transação**, idêntico para todas as linhas do mesmo commit). **Impacto real sobre o que o gestor vê: zero** — nenhum grupo empatado, em dev ou produção, tem veredito divergente; todos os 230 de produção são do mesmo check (`freshness_msc`) com o mesmo status. Era eleição por sorte que ainda não tinha dado errado. **Correção:** coluna `seq` alimentada por sequência — a **ordem de escrita** vira fato próprio, ao lado do carimbo, em vez de falsificar `executado_em` para forçar unicidade (alternativa considerada e recusada: o carimbo tem de continuar sendo o instante real da execução). Reescrever também avança o `seq`, senão o empate voltaria a decidir errado. A migration exigiu `GRANT USAGE` na sequência para a role da aplicação (padrão `APP_ROLE` das demais) — sem isso o *upsert* falha com `permission denied for sequence`, o que quebrou 6 testes da Sprint 26 até eu conceder; `downgrade` exercitado de verdade, não só escrito. Teste de regressão **força** o empate de carimbo em vez de esperá-lo do acaso. Verificado em produção após o deploy: 1.283 linhas com 1.283 `seq` distintos (desempate agora determinístico) e a role com `USAGE` confirmado. |
| 2026-08-13 | **Sprint IA-1a em produção — deploy verificado.** Primeira sprint do plano de IA/MCP (`docs/mcp_inteligencia_plataforma.md`). Backend em `79bfd52`; migrations `0043` (auditoria de chamada de ferramenta) e `0044` aplicadas juntas em passo isolado, exit 0; `/health` 200 local e via nginx; `alembic current` em `0044_dq_check_seq`. Sem frontend nesta sprint. Corrigido de quebra um defeito da H1 (de 2026-08-12): o teste do filtro de auditoria comparava `date.today()` **local** (UTC−3) com carimbos **UTC** e reprovava todo dia depois das 21h — reproduzido, diagnosticado com `TZ=UTC` e corrigido. |
